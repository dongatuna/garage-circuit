...
const CACHE = "garage-circuit-v10";
...
self.addEventListener("fetch", (e) => {
  ...
  if (isNavigation) {
    // Network-first for the app itself: online means you always get the current
    // build immediately. Falls back to cache only when offline.
    e.respondWith(
      fetch(e.request)
        .then((res) => {
